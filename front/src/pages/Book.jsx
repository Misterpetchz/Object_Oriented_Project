...
function Book() {
	...
	const add_rating = () => {
		axios
			.post(`http://localhost:8000/books/${params.bookname}/addrating`, {
				comment: comment,
				score: score_each_rating,
			})
			.then(setCommentSubmit(true))
			.catch(function (error) {
				console.log(error, "error");
			});
	};
	...
	const add_book_to_basket = () => {
		axios
			.post(
				`http://localhost:8000/books/${params.bookname}/add_book_to_basket?amount=1`
			)
			.then(() => {
				navigate("/basket");
			})
	...
	return (
		<div style={{ minHeight: 800, marginTop: 30 }}>
			...
			<br></br>
			<span>
				{book.available_branch?.map((branch) => (
			...
			<div>
				{rating.rating?.map((rate) => (
					<div>
						{rate.score_each_rating} {rate.comment}
					</div>
				))}{" "}
			</div>
			{CustomerOnlyButton() && (
